...
def integer_to_one_hot(integer, num_classes=10):
    # MNIST label one-hot-encoding n_classes = 10
...
y_train = integer_to_one_hot(y_train, num_classes=num_classes)
y_test  = integer_to_one_hot(y_test, num_classes=num_classes)

X_train = X_train[:1000]
y_train = y_train[:1000]

# === Encoder part ============
# requires two inputs (image and label) 
x_in  = Input(shape=entry_shape, name='entry')    # (28,28,1)
# label input and reshape
y_in  = Input(shape=label_shape, name='label')    # (10,)
y_enc = Dense(original_dim, activation=None)(y_in) #### sigmoid?
y_enc = Reshape(entry_shape)(y_enc)
# concatenated encoder input
x_enc = concatenate([x_in, y_enc])
# convolution
...
Encoder.summary()
# === Decoder part ============
z_in  = Input(shape=latent_dim, name='latent_in')
#y_dec = Dense(latent_dim, activation=None)(y_in)
#x_dec = concatenate([z_in, y_dec])
x_dec = concatenate([z_in, y_in]) # Do experiments
...
# --- build model
Decoder = Model([z_in, y_in], x_out, name='decoder')
Decoder.summary()
...
X_train_ori = X_train
y_train_ori = y_train
[z_mean, z_log_var, z] = Encoder([X_train_ori, y_train_ori])
z = np.array(z)
...
# === Single digit image generation

for i in range(10):
    # "i" is a digit to produce 
    z = np.random.normal(0, 1, size=latent_dim)
    z = z.reshape(1,latent_dim)
    y = integer_to_one_hot(i, num_classes=num_classes)
    y = y.reshape(1, num_classes)
    x_rec1 = Decoder.predict([z, y])
    z = np.random.normal(0, 1, size=latent_dim)
    z = z.reshape(1,latent_dim)
    y = integer_to_one_hot(i, num_classes=num_classes)
    y = y.reshape(1, num_classes)
    x_rec2 = Decoder.predict([z, y])
    view(x_rec1, x_rec2)

# === Feature Vector Manipulation
y = integer_to_one_hot(9, num_classes=num_classes)
y = y.reshape(1, num_classes)
z = np.zeros(latent_dim)
z = z.reshape(1, latent_dim)
X = Decoder.predict([z, y])
...
for i, yi in enumerate(grid_x):
    for j, xi in enumerate(grid_y):
        ...
        x_recons = Decoder.predict([z_sample, y])
